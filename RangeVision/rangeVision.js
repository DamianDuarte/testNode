...
function rangeVision(n) {
    const digits = n.toString();    
    let minVision = Infinity;    
    for (let i = 0; i < digits.length; i++) {
      const visionRange = parseInt(digits[i]);      
      const lDigit = digits.slice(Math.max(0, i - visionRange), i).split('');
      const rDigit = digits.slice(i + 1, i + 1 + visionRange).split('');      
      const visionValue = lDigit.concat(rDigit).reduce((sum, digit) => sum + parseInt(digit), 0);
      minVision = Math.min(minVision, visionValue);      

      if (digits[i] === '1' && visionValue === minVision) {
        return true;
      }
    }    
